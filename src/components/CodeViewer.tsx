...
export const CodeViewer: React.FC<CodeViewerProps> = ({
  ...
  const [isMobileContainer, setIsMobileContainer] = useState<boolean>(false);
        ...
        <div className="flex items-center gap-2 py-1.5">
          ...
          <button
            onClick={() => {
              ...
              setIsMobileContainer((prev) => !prev);
            }}
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all shadow-xs ${
              isMobileContainer && activeTab === "preview"
                ? "bg-indigo-600 border-indigo-600 text-white shadow-indigo-200"
                : "bg-white border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900"
            }`}
            title={
              isMobileContainer && activeTab === "preview"
                ? "Exit Mobile Container (Restore Full Canvas)"
                : "Wrap preview in mobile-sized container (375px)"
            }
            aria-pressed={isMobileContainer}
          >
            <Smartphone
              className={`w-3.5 h-3.5 transition-transform ${
                isMobileContainer && activeTab === "preview" ? "text-white scale-110" : "text-slate-500"
              }`}
            />
            <span>{isMobileContainer && activeTab === "preview" ? "Mobile Frame" : "Device View"}</span>
            <span
            ...
            >
              {isMobileContainer && activeTab === "preview" ? "375px" : "375px"}
            </span>
          </button>
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        {activeTab === "preview" && (
          <div className="h-full w-full overflow-hidden flex flex-col">
            {isMobileContainer ? (
              <div className="flex-1 bg-gradient-to-br from-slate-200 via-slate-100 to-slate-300 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 p-4 sm:p-6 overflow-y-auto flex flex-col items-center justify-start sm:justify-center">
                ...
                <div className="mb-3 flex items-center justify-between w-full max-w-[390px] px-2 text-xs text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-slate-800">Mobile Container</span>
                    <span className="font-mono text-[11px] bg-white/90 px-2 py-0.5 rounded border border-slate-300 shadow-2xs">
                      375 × 812 px
                    </span>
                  </div>
                  <button
                    onClick={() => setIsMobileContainer(false)}
                ...
                {/* Simulated Mobile Device Hardware Shell */}
                <div className="w-[375px] max-w-full h-[740px] max-h-[calc(100vh-175px)] min-h-[460px] bg-slate-950 rounded-[44px] p-3 shadow-2xl border-[6px] border-slate-800 ring-1 ring-slate-700/50 flex flex-col relative shrink-0 transition-all duration-300">
                  {/* Speaker & Dynamic Island */}
                  <div className="h-6 flex items-center justify-between px-6 text-white text-[11px] font-semibold select-none shrink-0 mb-1.5">
                    <span>9:41</span>
                    <div className="w-24 h-4 bg-black rounded-full border border-slate-800 flex items-center justify-center gap-1.5 shadow-inner">
                      ...
                      <div className="w-2 h-2 rounded-full bg-indigo-950 border border-slate-800"></div>
                    </div>
                    <div className="flex items-center gap-1.5 text-slate-300">
                      <span className="text-[10px]">5G</span>
                      <div className="w-4 h-2 rounded-xs border border-white/60 p-0.5 flex items-center">
                        <div className="h-full w-2.5 bg-emerald-400 rounded-2xs"></div>
                      </div>
                    </div>
                  </div>

                  {/* Device Screen Frame */}
                  <div className="flex-1 w-full rounded-[28px] overflow-hidden bg-white shadow-inner flex flex-col relative min-h-0">
                    <PreviewCanvas
                  ...
                  <div className="pt-2 pb-1 flex justify-center shrink-0">
                    <div className="w-32 h-1 bg-slate-600 rounded-full select-none"></div>
                  </div>
                </div>
              </div>
            ) : (
              <PreviewCanvas
                ...
                onToggleMobile={() => setIsMobileContainer(true)}
              />
            )}
          </div>
        )}
